...
# 4. TF-IDF 벡터화
def build_tfidf(df_clean, target_col = 'content_clean') :
    '''
...
def check_title_content_overlap(df):
    """
    제목의 단어들이 본문에 얼마나 포함되어 있는지 비율을 계산합니다.
    """
    total_overlap_ratio = []
    
    for _, row in df.iterrows():
        # 전처리된 텍스트 활용 (공백 기준 split)
        title_words = set(str(row['title']).lower().split())
        content_words = set(str(row['content_clean']).split())
        
        # 제목 단어 중 본문에 있는 단어 계산
        overlap = title_words & content_words
        
        # 제목이 비어있는 경우 제외
        if len(title_words) > 0:
            ratio = len(overlap) / len(title_words)
            total_overlap_ratio.append(ratio)
    
    avg_ratio = np.mean(total_overlap_ratio)
    print(f"=== 제목 단어와 본문 단어 중복 분석 ===")
    print(f"평균적으로 제목 단어의 {avg_ratio*100:.2f}%가 본문에 이미 포함되어 있습니다.")
    return avg_ratio
...
def main() :
    ...
    overlap_rate = check_title_content_overlap(df_clean)
    print(overlap_rate)
    ...
    tfidf_matrix_imp, vectorizer_imp = build_tfidf(df_imp, target_col = 'content_weighted')
    ...
    # main() 함수 하단, 출력 함수 호출하는 곳 근처에 추가하세요.

    # 1. 비교 로직 시작
    print("=== [분석] 성능 변화가 있는 쿼리 확인 ===")
    
    for i in range(len(keyword_base_logs)):
        # 로그에서 검색된 ID 가져오기
        base_res = keyword_base_logs[i]['retrieved_ids']
        imp_res = keyword_imp_logs[i]['retrieved_ids']
        
        # 만약 결과가 다르다면?
        if base_res != imp_res:
            print(f"{i}번 변화 발생! 질문: {keyword_base_logs[i]['question']}")
            print(f" - Base 결과: {base_res}")
            print(f" - Imp 결과: {imp_res}")

    # 1. 비교 로직 시작
    print("=== [분석] 성능 변화가 있는 쿼리 확인 ===")
    counts = 0
    for i in range(len(tfidf_base_logs)):
        # 로그에서 검색된 ID 가져오기
        base_res = tfidf_base_logs[i]['retrieved_ids']
        imp_res = tfidf_imp_logs[i]['retrieved_ids']
        
        # 만약 결과가 다르다면?
        if base_res != imp_res:
            print(f"{i}번 변화 발생! 질문: {tfidf_base_logs[i]['question']}")
            print(f" - Base 결과: {base_res}")
            print(f" - Imp 결과: {imp_res}")
            counts += 1
